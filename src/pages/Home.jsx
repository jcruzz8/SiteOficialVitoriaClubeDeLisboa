...
const Home = () => {
  ...
  // --- LÓGICA DO COMUNICADO DE DESTAQUE (SUPABASE) ---
  useEffect(() => {
    const fetchUltimoComunicado = async () => {
      try {
        // Vai à tabela, ordena pela data mais recente e pede só 1 (.limit(1))
        ...
        // Se encontrou dados, guarda o primeiro (e único) resultado
        if (data && data.length > 0) {
          setUltimoComunicado(data[0]);
        }
      } catch (err) {
        console.error("Erro ao carregar último comunicado:", err.message);
      }
    };

    fetchUltimoComunicado();
  }, []);

  // --- LÓGICA DO TEMPORIZADOR (AUTOPLAY JOGOS) ---
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Helmet>
        <title>Vitória Clube de Lisboa | Home</title>
      ...
      {/* --- 4. SECÇÃO: DESTAQUE / COMUNICADOS (DARK MODE) --- */}
      {ultimoComunicado && (
        <section className="py-24 relative overflow-hidden bg-zinc-800">
          ...
          {/* Elementos Decorativos Animados e Grelha */}
          <div className="absolute inset-0 z-0">
            <div className="absolute -top-40 -left-40 w-80 h-80 bg-vcl-red rounded-full opacity-5 blur-3xl animate-pulse"></div>
            <div className="absolute -bottom-40 -right-40 w-80 h-80 bg-vcl-gold rounded-full opacity-3 blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>

            <svg className="absolute inset-0 w-full h-full opacity-[0.01]" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <pattern id="modernGridNews" x="0" y="0" width="50" height="50" patternUnits="userSpaceOnUse">
                  <circle cx="25" cy="25" r="1" fill="#fff" />
                  <path d="M 0 0 L 50 50 M 50 0 L 0 50" stroke="#fff" strokeWidth="0.5" />
                </pattern>
              </defs>
              <rect width="100%" height="100%" fill="url(#modernGridNews)" />
            </svg>
          </div>
          ...
          <div className="max-w-5xl mx-auto px-4 relative z-10">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-3xl font-black text-white uppercase flex items-center gap-3">
                <Newspaper className="text-vcl-red" size={32} />
                Última Hora
              </h2>
              <Link to="/comunicados" className="text-vcl-red font-bold hover:text-white transition hidden sm:block">
                Ver todos os comunicados
              </Link>
            </div>

            <div className="bg-zinc-800/60 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row group hover:shadow-[0_0_30px_rgba(230,0,0,0.15)] transition duration-300 border border-zinc-700 hover:border-vcl-red/50">
              {ultimoComunicado.imagem && (
                <div className="md:w-5/12 h-64 md:h-auto overflow-hidden relative">
                  <img src={ultimoComunicado.imagem} alt="Comunicado" className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                  {/* Overlay sutil na imagem para mobile */}
                  <div className="absolute inset-0 bg-gradient-to-t from-zinc-900/90 to-transparent md:hidden"></div>
                </div>
              )}
              <div className="p-8 md:w-7/12 flex flex-col justify-center">
                <div className="flex items-center gap-2 text-gray-400 text-xs font-bold uppercase mb-3">
                  <Calendar size={16} /> {ultimoComunicado.data}
                </div>
                <h3 className="text-2xl md:text-3xl font-bold text-white mb-4 line-clamp-2 group-hover:text-vcl-red transition leading-tight">
                  {ultimoComunicado.titulo}
                </h3>
                <div
                  className="text-gray-300 mb-6 line-clamp-3 leading-relaxed text-sm [&>p]:mb-0 [&_*]:!text-gray-200"
                  dangerouslySetInnerHTML={{ __html: ultimoComunicado.conteudo }}
                />
                <Link to="/comunicados" className="inline-flex items-center gap-2 bg-vcl-red text-white px-6 py-3 rounded-lg font-bold uppercase hover:bg-red-700 transition w-fit mt-auto shadow-lg hover:shadow-red-500/30">
                  Ler Comunicado <ArrowRight size={18} />
                </Link>
              </div>
            </div>

            <div className="mt-6 text-center sm:hidden">
              <Link to="/comunicados" className="text-vcl-red font-bold hover:text-white transition">
                Ver todos os comunicados
              </Link>
            </div>
          </div>
